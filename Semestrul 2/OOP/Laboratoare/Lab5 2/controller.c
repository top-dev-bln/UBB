...
#include "dynamic_list.h" // Added include for DynamicList
...
Controller Creeaza_Controller(Repository* repo)
{
	...
	Controller ctrl;
	ctrl.repo = repo;
	ctrl.undoStack = CreateDynamicList();
	return ctrl;
}
...
int Adauga_Oferta_Controller(Controller* ctrl, const char* tip, int suprafata, const char* adresa, int pret)
{
	...
	AddToList(ctrl->undoStack, Get_All(ctrl->repo));
...
int Undo(Controller* ctrl) {
	if (ctrl->undoStack->size == 0) {
		return 0;
	}


	Repository* previousState = (Repository*)GetFromList(ctrl->undoStack, ctrl->undoStack->size - 1);


	Repository* newRepo = Get_All(previousState);
	...
	RemoveFromList(ctrl->undoStack, ctrl->undoStack->size - 1, NULL);
